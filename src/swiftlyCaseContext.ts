...
export const SWIFTLY_CASE_CONTEXT: string = [
  ...
  `- Shipped decisions: (1) one live view replacing the daily paper report; (2) triage-first information hierarchy — rationale (Joanna's own reasoning, not a user quote): "We considered the common F-shaped reading pattern for English-speaking users. The first column prioritizes task triage, with the vehicle's in-service status being the most critical element."; (3) a color legend over icon-only status, weighed against visual complexity for a fast-scanning ops audience; (4) hover to monitor, click to investigate — framed as "Should we prioritize monitoring or investigating?", hover won so glanceable status stays the default and going deeper is optional.`,
  `- Impact: device investigation time fell from 30+ hours to 12-24 hours. Device-issue inbound to Client Success dropped 20%. These are two DIFFERENT cross-functional measurements: investigation time is IT's end-to-end metric per device issue; the inbound drop is Client Success's volume metric. The deck also phrases the first as "investigation time reduced by 20%" — that is the conservative bound of the same 30+ to ~24-hour improvement, not the inbound number. The internal goal of under 12 hours was NOT met — an honest miss that taught the team a single investigation-time target measured the wrong thing, since agencies triage differently. Beyond metrics: improved organizational scalability, and my manager credited the project with moving the Data Monitor team from engineering-led to design-led planning.`,
