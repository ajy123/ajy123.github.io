...
export const DEELI_CASE_CONTEXT: string = [
  ...
  `- Deeli: an enterprise deep-tech research product. Client: a top-5 semiconductor foundry. Joanna: only designer, research to launch, 5-engineer build. Launched June 2026; impact measured over the first 2 weeks live.`,
  ...
  `- #ns-method "The story above ran on a working method." Published: persona.md is built from interviews, user feedback and usage data, the same weekly pipeline ranks feedback into features, and upkeep that took "6+ hours a week now takes about one"; an AI judge grades every answer against the persona questions, so review asks which check failed instead of arguing taste; the interface is written as a few design primitives in design.md, and the doc is the handoff, engineers read it instead of meeting. UNPUBLISHED, do not state to visitors: those primitives are content not chrome, encode by exception, hierarchy via weight and space not color.`,
