...
export const DEELI_CASE_CONTEXT: string = [
  ...
  `- #ns-method "The story above ran on a working method." Published: persona.md is built from interviews, user feedback and usage data, the same weekly pipeline ranks it into features, and upkeep that took "6+ hours a week now takes about one"; an AI judge grades every answer against the persona questions, so review asks which check failed instead of arguing taste; the interface is written as a few design primitives in design.md, and the doc is the handoff, engineers read it instead of meeting. UNPUBLISHED, do not state to visitors: those primitives are content not chrome, encode by exception, hierarchy via weight and space not color.`,
